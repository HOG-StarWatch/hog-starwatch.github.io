...
*   **🎨 极光设计 (Aurora Design)**: 全局统一的 CSS 变量系统，深度应用玻璃拟态 (`backdrop-filter`) 与动态渐变，支持深色模式与响应式布局（移动端/桌面端完美适配）。

---

## 🛠️ 功能清单 (Tool List)

本项目包含 **25+** 个独立工具，覆盖开发、设计、运维等多个场景：
...
*   **UI**: 纯手搓自研 纯 CSS 实现，无任何 UI 框架依赖，轻量且易于定制。
