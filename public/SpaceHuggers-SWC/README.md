...
在原作 Space Huggers 的基础上添加“作弊菜单”的学习型改造版本。改动集中在 cheatMenu.js，默认保持其他文件与原版一致。

## 快速开始
- 直接用浏览器打开 `index.html` 即可游玩。
- 启用作弊菜单（不改动原版文件）：在浏览器控制台执行以下代码加载 `cheatMenu.js`。
...
- 需要长期启用时，可在 `index.html` 末尾追加 `<script src="cheatMenu.js"></script>`。

- 控制台本地粘贴版（避免符号解析问题，粘贴 Base64 内容）：
...
```js
const s = document.createElement('script');
s.src = 'https://your-domain/cheatMenu.js';
document.body.appendChild(s);
```

- 控制台一键注入版本（远程托管后可直接粘贴运行）：

```js
(() => {
  if (window.CheatMenuLoaded) return;
  const s = document.createElement('script');
  s.src = 'https://your-domain/cheatMenu.js?ts=' + Date.now();
  document.body.appendChild(s);
})();
```

- 如果站点启用了 CSP，需将 `cheatMenu.js` 放到同源并允许脚本加载，或用用户脚本工具注入。

## 改版原则
- 所有改动集中在 `cheatMenu.js`。
- 其他文件与原版一致，方便随时还原。
...
- 战斗辅助：一击必杀、极速射击、爆炸子弹、无限手雷、取消手雷冷却
- 世界控制：移动/射速/重力/爆炸冲击与点燃范围等倍率调节、关闭粒子/音效、渲染爆炸半径与手雷轨迹
- 地形编辑：前景/背景图层、笔刷大小、快速绘制并异步重绘
- 生成器：在鼠标处生成敌人/箱子/补给，或在鼠标附近生成选中项
...
### 原作简介
帝国像瘟疫一样在银河系蔓延，并在偏远星球上建立前哨基地。作为一名精英反叛军士兵，你的任务是消灭这些基地。使用你的毁灭工具探索奇异的星球并消灭入侵者！你只剩下 10 个克隆体，每次任务后会补充 3 个。祝你好运，玩得开心，替我拥抱太空。
